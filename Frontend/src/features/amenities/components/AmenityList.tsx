...
export default function AmenityList() {
  ...
  const columns: Column<Amenity>[] = [
    { 
      header: 'Nombre', 
      accessor: (row) => <span className="font-bold text-[var(--foreground)]">{row.nombre}</span>
    },
    { 
      header: 'Complejo', 
      accessor: (row) => <span className="font-medium text-blue-600 dark:text-blue-400">{row.nombreComplejo}</span> 
    },
    { header: 'Capacidad', accessor: (row) => `${row.capacidad} personas` },
    { 
      header: 'Estado', 
      accessor: (row) => (
        <span className={`px-2 py-1 rounded-md text-xs font-medium ${
          row.estado === 'ACTIVO' ? 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400' :
          row.estado === 'MANTENIMIENTO' ? 'bg-amber-100 text-amber-800 dark:bg-amber-900/30 dark:text-amber-400' :
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold tracking-tight text-[var(--foreground)]">Amenities</h2>
          <p className="text-sm text-gray-500">
            Administración de espacios comunes (SUM, parrillas, piscina, etc).
          </p>
        </div>
        <button
      ...
      >
        <div className="space-y-4">
          <p className="text-sm text-gray-600 dark:text-gray-400">
            ¿Estás seguro que deseas eliminar el amenity{' '}
            <strong className="text-[var(--foreground)]">"{selectedAmenity?.nombre}"</strong>?
          </p>
