...
export default function AuditLogList() {
    ...
    { 
      ...
      accessor: (row) => (
        <span className={`px-2.5 py-1 rounded-lg text-xs font-semibold border ${
          row.accion === 'CREATE' ? 'bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border-emerald-500/20' :
          row.accion === 'UPDATE' ? 'bg-blue-500/10 text-blue-600 dark:text-blue-400 border-blue-500/20' :
          row.accion === 'DELETE' ? 'bg-red-500/10 text-red-600 dark:text-red-400 border-red-500/20' :
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      {/* Header Controls */}
      <div className="pb-2 border-b border-brand-surface-bright/20">
        ...
        <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
          Historial completo de acciones y modificaciones realizadas en el sistema.
        </p>
      </div>
