      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 pb-2 border-b border-brand-surface-bright/20">
        <div>
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
            Administración de inquilinos y residentes vinculados a cada unidad.
          </p>
        </div>
        
        <CreateButton label="Nuevo Inquilino" onClick={handleOpenCreate} />
      </div>
      ...
      >
        <div className="space-y-4">
          <p className="text-xs text-slate-600 dark:text-slate-400 leading-relaxed">
            ¿Estás seguro de que deseas eliminar al inquilino{' '}
            <strong className="text-slate-800 dark:text-slate-100 font-bold">{selectedInquilino?.nombre} {selectedInquilino?.apellido}</strong>?
          </p>
          <div className="flex gap-3 pt-4 border-t border-brand-surface-bright/10 dark:border-white/5">
            ...
            >
              {isSubmitLoading ? 'Eliminando...' : 'Eliminar Inquilino'}
            </button>
          </div>
        </div>
      </Modal>
